...
function renderHealth(data){
  ...
  c.innerHTML='<div class="portal-medical-view medical-records">'+recs.map(function(r){var p=r.patient||{},m=r.medical_profile||{};return '<article class="medical-record-card">'+recordHead(r)+'<section class="medical-section"><h3>البيانات الأساسية</h3><div class="health-grid">'+item("رقم الهاتف",p.phone,true)+item("البريد الإلكتروني",p.email)+item("السن",p.age)+item("النوع",gender(p.gender))+item("الطول",m.height)+'</div></section><section class="medical-section"><h3>التاريخ الصحي</h3><p class="medical-section-note">الطبيب والقياسات تظهر داخل كل زيارة.</p></section><section class="medical-section"><h3>الأمراض المزمنة والأدوية المرتبطة</h3>'+chips(m.chronic_conditions,"chronic")+'</section><section class="medical-section"><h3>العمليات الجراحية</h3>'+chips(m.surgeries,"surgery")+'</section><section class="medical-section"><h3>التاريخ المرضي بالعائلة</h3>'+chips(m.family_history,"family")+'</section></article>';}).join("")+'</div>';
}
...
function renderFollowup(data){
  ...
  c.innerHTML='<div class="portal-medical-view medical-records">'+recs.map(function(r){
    var f=r.next_follow_up,p=r.patient||{},latest=r.latest_visit||{};
    var status=f&&f.follow_up_status;
    var title=status==="attended"?"تم حضور المتابعة":status==="no_show"?"لم يتم الحضور للمتابعة":"متابعة طبية قادمة";
    var note=status==="attended"?"تم تسجيل حضور المتابعة من المركز.":status==="no_show"?"تم تسجيل عدم الحضور من المركز.":"موعد المتابعة مسجل من آخر خطة علاجية في ملفك.";
    var lead=status==="attended"?'<div class="followup-status-box">تم الحضور</div>':status==="no_show"?'<div class="followup-status-box">لم يتم الحضور</div>':'<div class="followup-date-box"><strong>'+fmtDate(f.follow_up_date)+'</strong><span>الموعد المسجل</span></div>';
    return '<article class="medical-record-card">'+recordHead(r)+'<section class="medical-section"><h3 class="upcoming-followup-title">المتابعات القادمة</h3>'+
      (f?'<div class="followup-card '+(status==="no_show"?"followup-no-show":status==="attended"?"followup-attended":"")+'">'+lead+'<div class="followup-copy"><h3>'+title+'</h3><p>'+note+'</p>'+(f.complaint?'<p class="complaint">الشكوى المسجلة: '+esc(f.complaint)+'</p>':'')+'</div></div>':'<div class="medical-empty">لا يوجد موعد متابعة محدد حاليًا.</div>')+
      '</section><section class="medical-section"><h3>آخر زيارة مسجلة</h3><div class="health-grid">'+item("تاريخ آخر زيارة",latest.visit_date||p.last_visit_date,true)+item("رقم الزيارة",latest.visit_number)+item("ضغط الدم",latest.blood_pressure)+item("سكر الدم",latest.blood_sugar)+'</div></section></article>';
  }).join("")+'</div>';
}
