...
-- ============================================================
-- قسم ٥٣: Patient Portal — Email Activation (Phase 2B)
-- ٢٠٢٦-٠٩-١٠
--
-- الهدف:
-- 1) إلغاء الاعتماد على Phone OTP/Twilio.
-- 2) البريد المسجل داخل ملف المريض هو عامل الوصول المستقل.
-- 3) المريض يرسل طلب + مستندات، الموظف يراجعها.
-- 4) عند الاعتماد يتولد كود تفعيل لمرة واحدة ويُرسل من بريد Swnw.
-- 5) المريض يدخل الكود ويحدد كلمة سر جديدة؛ بعدها الدخول Email + Password.
-- 6) كود التفعيل لا يُخزن كنص صريح — SHA-256 فقط.
-- ============================================================

-- البريد التشغيلي المسجل على ملف المريض. نفس البريد ممكن يتكرر على أكتر من
-- ملف (مثلاً أب/أم + أطفال)، لأن حساب Portal واحد ممكن يوصل لعدة patients.id.
alter table public.patients
...
-- حساب Portal: بريد دخول واحد لكل حساب + حالة pending_activation قبل أول تفعيل.
...
-- لو فيه حسابات تجريبية قديمة من نسخة Phone OTP، حاول نملأ البريد من auth.users
-- من غير ما نغيّر أي حساب ملوش بريد.
update public.patient_accounts a
set login_email = lower(trim(u.email))
from auth.users u
where a.auth_user_id = u.id
  and a.login_email is null
...
create unique index if not exists patient_accounts_login_email_uidx
  on public.patient_accounts (lower(login_email))
  where login_email is not null;

-- Snapshot للبريد المستخدم في الطلب + token مؤقت لرفع المستندات قبل تفعيل الحساب.
alter table public.patient_identity_verifications
  add column if not exists requested_email text;
alter table public.patient_identity_verifications
  add column if not exists submission_token_hash text;
alter table public.patient_identity_verifications
  add column if not exists submission_token_expires_at timestamptz;

-- أكواد التفعيل: الهاش فقط، صلاحية قصيرة، ومحاولات محدودة.
create table if not exists public.patient_activation_codes (
  id               uuid primary key default gen_random_uuid(),
  account_id       uuid not null references public.patient_accounts(id) on delete cascade,
  verification_id  uuid references public.patient_identity_verifications(id) on delete set null,
  code_hash        text not null,
  expires_at       timestamptz not null,
  used_at          timestamptz,
...
create index if not exists patient_activation_codes_account_idx
  on public.patient_activation_codes (account_id, created_at desc);
create index if not exists patient_activation_codes_active_idx
  on public.patient_activation_codes (account_id, expires_at)
  where used_at is null;

alter table public.patient_activation_codes enable row level security;
-- مقصود: مفيش policies للـclient. القراءة/الكتابة حصريًا عن طريق Edge Functions
-- بمفتاح service_role.

-- تدفق الرفع العام الجديد بيعدّي من Edge Function بمفتاح service_role.
-- لو insert حصل من مستخدم authenticated قديم، حافظ على السلوك القديم؛
-- ولو من service_role اسمح للباك إند يثبت account_id كـuploaded_by.
create or replace function public.enforce_verification_document_uploader()
returns trigger language plpgsql security definer set search_path = public as $$
declare v_account_id uuid;
begin
...
-- الاعتماد يرفض نهائيًا لو بريد حساب Portal لا يطابق البريد المسجل على
-- ملف المريض. ده إنفاذ DB-level، مش مجرد check في الواجهة/Edge Function.
create or replace function public.approve_patient_identity_verification(p_verification_id uuid)
returns public.patient_account_access
language plpgsql security definer set search_path = public as $$
declare
...
begin
  ...
  select * into v_verification
    from public.patient_identity_verifications
    where id = p_verification_id
    for update;
  ...
  select lower(trim(login_email)) into v_account_email
    from public.patient_accounts
    where id = v_verification.account_id;

  select lower(trim(email)) into v_patient_email
    from public.patients
    where id = v_verification.requested_patient_id;

  if v_account_email is null or v_patient_email is null or v_account_email <> v_patient_email then
    raise exception 'patient portal email does not match the verified patient-record email';
  end if;
  ...
  update public.patient_identity_verifications
    set status = 'approved',
        reviewed_by = public.my_admin_id(),
        reviewed_at = now(),
        updated_at = now()
    where id = p_verification_id;
...
-- ملاحظة تشغيل:
-- البريد لا يتغير تلقائيًا من طلب المريض. الموظف يراجع/يسجل البريد في patients.email
-- من الداشبورد أولًا. الاعتماد مستحيل لو البريد غير موجود أو غير مطابق.
-- ============================================================
