...
alter table public.content_items
  add column if not exists video_music_mood text not null default 'calm'
  check (video_music_mood in ('calm', 'upbeat', 'serious'));

alter table public.video_jobs
  add column if not exists music_mood text not null default 'calm'
  check (music_mood in ('calm', 'upbeat', 'serious'));
...
begin
  ...
  if not found then
    raise exception 'احفظ لوجو هذا البراند في لوحة الإدارة أولاً';
  end if;
  ...
  if existing_job.id is not null then
    update public.video_jobs set
      title=item.title,brand=item.brand,specialty=item.specialty,
      ...
      video_template=item.video_template,media_mode=item.video_media_mode,
      music_mood=coalesce(item.video_music_mood, 'calm'),
      input_assets=asset_snapshot,cover_settings=settings,input_schema_version=2
    where id=existing_job.id and status='pending'
