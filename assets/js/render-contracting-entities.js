...
(function () {
  ...
  function openCreate(container) {
    ...
    var bd = modal("إضافة جهة تعاقد", '<div class="section" style="margin:0;"><h4>بيانات الجهة</h4>' + input("ce-name", "اسم المدرسة / المركز", "text") + '<div class="field"><label>نوع الجهة</label><select id="ce-type">' + typeOpts + '</select></div>' + input("ce-address", "العنوان", "text") +
      ...
      '<div class="field"><label>اسم مسؤول التعاقد</label><input value="' + esc((me && (me.name || me.email)) || "") + '" readonly></div>' + input("ce-signed-by", "اسم الموقّع على العقد", "text") + input("ce-signed-doc", "رابط/مرجع العقد الموقّع", "url") +
    ...
    bd.querySelector("#ce-save").onclick = function () {
      ...
      window.SSMPDDb.createContractingEntity(entity, contact, contract, activity).then(function () { T.show("تم حفظ جهة التعاقد"); bd.remove(); load(container); }).catch(function (e) { bd.querySelector("#ce-save").disabled = false; T.show("خطأ: " + e.message, "error"); });
    };
  }
  ...
  function openDetails(entityId, container) {
    ...
    function reload() {
      window.SSMPDDb.getContractingEntityDetails(entityId).then(function (d) {
        ...
        html += '<div class="section"><h4>العقود</h4><div style="overflow:auto"><table><thead><tr><th>الحالة</th><th>من</th><th>إلى</th><th>خصم</th><th>كاش باك</th><th>الخدمات</th></tr></thead><tbody>' + d.contracts.map(function (c) { return '<tr><td>' + statusPill(c.status) + '</td><td>' + date(c.start_date) + '</td><td>' + date(c.end_date) + '</td><td>' + c.discount_percent + '%</td><td>' + c.cashback_percent + '%</td><td>' + esc(c.services || "—") + '</td></tr>'; }).join("") + '</tbody></table></div></div>';
