...
(async()=>{
 ...
 const daily=await run({op:'set_daily_completion',tracking_date:'2026-09-01'});assert.equal(daily.status,200);assert.equal(daily.writes.length,1);assert.equal(daily.writes[0].table,'patient_nutrition_daily_completions');assert.equal(daily.writes[0].data.tracking_date,'2026-09-01');
 for(const day of ['2025-12-31','2026-02-30','2099-01-01','bad']){const r=await run({op:'set_daily_completion',tracking_date:day});assert.equal(r.status,400);assert.equal(r.writes.length,0);}
 for(const opts of [{patient:'other'},{access:{revoked_at:'2026-01-01'}},{access:{expires_at:'2020-01-01'}}]){const r=await run({op:'set_daily_completion',tracking_date:'2026-09-01'},opts);assert.equal(r.status,403);assert.equal(r.writes.length,0);}
 const dayRead=await run({op:'daily_overview',tracking_date:'2026-09-01'});assert.equal(dayRead.status,200);assert.equal(dayRead.body.tracking_date,'2026-09-01');assert.equal(dayRead.body.visits[0].completions.length,1);assert.equal(dayRead.body.visits[0].completions[0].completed,true);
